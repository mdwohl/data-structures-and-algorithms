...
const createServer = () => {
  ...
  function greet(request, response){
    response.send('well hi!')
  }

  function aboutMe(request, response){
    response.send('semi-autonomous carbon unit')
  }

  function foodFunction(request, response){
    const foodArr = ['seafood, noodles, pretzels'];
    response.send(foodArr);
  }

  app.get('/hello', greet);
  app.get('/aboutme', aboutMe);
  app.get('/favoritefoods', foodFunction);
...
const count = (target, input) => {
  ...
  input.map(insideArray => {
    insideArray.map(element => {
      if(element === target){
        instanceCounter++;
      }
    }); 
...
const totalSum = (input) => {
    let arrSum = 0;
    input.reduce(insideArray => {
      insideArray.reduce(element => {
        if(element === target){
          let arrSum = insideArray;
        }
      }); 
    });
    return arrSum;
