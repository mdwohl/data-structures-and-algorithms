...
const toLastNames = people => {
  ...
  array1.push(value.firstName + space + value.lastName);
...
let regexSearch = /\b\d{4}\b/;
if (regexSearch.test(pin)){
  return true;
}else{
  return false;
}
...
const validateEmail = (email) => {
  let regexKey = /^\w+(|.)\w+@\w+.(com|net|org)$/;
  if (regexKey.test(email)){
    return regexKey.test(email);
  }else{
    return false;
  }
